...
コメント機能を作るだけではなく、**通知センター + 自分の投稿/コメント履歴を1か所で確認できるマイアクティビティ画面**を用意する。
...
単なる通知一覧とは別に、自分が過去に書いた内容や、自分宛てに来た内容をいつでも見返せる画面を作る。
...
未読件数をタブbadgeで表示できるようにする。

### あなたのコメント

自分が過去に書いたコメント/回答を一覧表示する。
...
これにより、通知を既読にしてしまった後でも自分の活動履歴から追えるようにする。
...
```text
[あなたへのコメント 3] [あなたのコメント] [あなたの質問 1] [投稿編成]
...
通知または活動履歴をタップすると対象コメント/質問位置までscroll/highlightできるようにする。
...
- [ ] notification schema
- [ ] `/notifications` route
- [ ] `/me/activity` routeまたは同等のマイアクティビティ画面
- [ ] `あなたへのコメント` タブ
- [ ] `あなたのコメント` タブ
- [ ] `あなたの質問` タブ
- [ ] `あなたの投稿編成` タブ
- [ ] 投稿編成ごとのコメント/質問/未読件数表示
- [ ] 自分のコメント履歴から元コメントへ遷移
- [ ] ベルアイコン + 未読badge
- [ ] 未読/既読
- [ ] すべて既読
